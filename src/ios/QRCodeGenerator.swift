...
//  HelloWorld
//
//  Created by Eugene Bokhan on 10/01/2019.
//
...
public class QRCodeGenerator : NSObject {
    ...
    public func createCGImage(value: String) -> CGImage? {
        let stringData = value.data(using: String.Encoding.isoLatin1, allowLossyConversion: true)
        ...
        scaleFilter.setValue(qrFilter.value(forKey: "outputImage"), forKey: "inputImage")
        scaleFilter.setValue(20, forKey: "inputScale")
        scaleFilter.setValue(1.0, forKey: "inputAspectRatio")
        
        guard let filterOutputImage = outputImageFromFilter(filter: scaleFilter) else { return nil }
        guard let outputImage = imageWithImageFilter(inputImage: filterOutputImage) else { return nil }
        ...
        guard let cgImage = CIContext(options: contextOptions).createCGImage(outputImage, from: outputImage.extent) else { return nil }
        return cgImage
    }
